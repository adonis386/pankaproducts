...
export default function CheckoutPage() {
    ...
    return (
      <section className="flex min-h-[70vh] items-center justify-center py-20">
        <div className="mx-auto max-w-sm px-6 text-center animate-scale-in">
          ...
          <h1 className="mb-2 font-heading text-2xl font-bold text-panka-brown-500">
    ...
    return (
      <section className="flex min-h-[70vh] items-center justify-center py-20">
        <div className="mx-auto max-w-sm px-6 text-center">
          ...
          <h1 className="mb-2 font-heading text-xl font-bold text-panka-brown-500">{t("checkout.emptyCart")}</h1>
          <p className="mb-6 text-sm text-grey-40">{t("checkout.emptyCartDesc")}</p>
          <Link href="/productos" className="inline-block rounded-xl bg-panka-brown-500 px-7 py-3 text-sm font-bold text-white">
  ...
  const inputClass =
    "w-full rounded-xl border border-grey-20 bg-grey-5 px-4 py-3 text-sm text-grey-80 outline-none transition-all placeholder:text-grey-30 focus:border-panka-green-400 focus:ring-2 focus:ring-panka-green-50";

  return (
    <>
      <section className="border-b border-grey-10 bg-white pb-6 pt-10">
        <div className="mx-auto max-w-7xl px-6">
          <Link href="/productos" className="mb-3 inline-flex items-center gap-1.5 text-xs text-grey-40 transition-colors hover:text-grey-60">
            <HiArrowLeft className="h-3.5 w-3.5" />
            {t("checkout.continueShopping")}
          </Link>
          <h1 className="font-heading text-2xl font-bold text-panka-brown-500">{t("checkout.title")}</h1>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 gap-8 small:grid-cols-5">
            <form onSubmit={handleSubmit} className="small:col-span-3">
              <div className="rounded-2xl border border-grey-10 bg-white p-6 small:p-8">
                <h2 className="mb-6 text-sm font-bold text-grey-80">{t("checkout.deliveryDetails")}</h2>
                <div className="grid grid-cols-1 gap-4 xsmall:grid-cols-2">
                  <div className="xsmall:col-span-2">
                    <label className="mb-1.5 block text-xs font-medium text-grey-50">{t("checkout.fullName")}</label>
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-medium text-grey-50">{t("checkout.email")}</label>
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-medium text-grey-50">{t("checkout.phone")}</label>
                    <input type="tel" name="phone" value={form.phone} onChange={handleChange} required className={inputClass} placeholder="(305) 123-4567" />
                  </div>
                  <div className="xsmall:col-span-2">
                    <label className="mb-1.5 block text-xs font-medium text-grey-50">{t("checkout.address")}</label>
                  ...
                  <div className="xsmall:col-span-2">
                    <label className="mb-1.5 block text-xs font-medium text-grey-50">{t("checkout.city")}</label>
                  ...
                  <div className="xsmall:col-span-2">
                    <label className="mb-1.5 block text-xs font-medium text-grey-50">{t("checkout.notes")}</label>
                ...
                <button type="submit" disabled={isSubmitting} className="mt-6 w-full rounded-xl bg-panka-brown-500 py-3.5 text-sm font-bold text-white transition-all hover:bg-panka-brown-600 hover:shadow-panka-md disabled:opacity-50">
            ...
            <div className="small:col-span-2">
              <div className="sticky top-24 rounded-2xl border border-grey-10 bg-white p-6">
                <h2 className="mb-5 text-sm font-bold text-grey-80">{t("checkout.orderSummary")}</h2>
                  ...
                  {items.map((item) => (
                    <li key={item.product.id} className="flex gap-3">
                      ...
                      <div className="flex flex-1 items-center justify-between">
                        <div>
                          <p className="text-[13px] font-medium text-grey-70 leading-tight">{item.product.name}</p>
                ...
                <div className="border-t border-grey-10 pt-4">
                  <div className="mb-1.5 flex justify-between text-xs text-grey-40">
                  ...
                  <div className="mt-3 flex justify-between border-t border-grey-10 pt-3">
                    <span className="text-sm font-bold text-grey-80">{t("checkout.total")}</span>
                    <span className="text-sm font-bold text-grey-80">${totalPrice.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
